...
;
;when testing, the simulator should be reinitialized for each run
;
;input should start with 0 if a score is less than 100 (i.e. 077, 099, 054,...) for grading to display correct grade 
; 
.ORIG x3000
...
MAX	ST R0, SAVER0		;SAVE R0 just in case for save restore operations requirement
	ST R7, RETADDRESS
	AND R5, R5, #0
...
EXIT1	LD R1, SCORES1	;output of max score
	...
	AND R7, R7, #0
	AND R0, R0, #0
	LD R0, SAVER0		;SAVE R0 just in case for save restore operations requirement
	LD R7, RETADDRESS
	RET
